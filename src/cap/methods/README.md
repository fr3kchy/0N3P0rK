...
**PACK** — это **как интенсивно** (параметры kick'а).

В настройках радио (`R`) есть пункт `PACK`:

| Pack | Что делает |
|---|---|
| `STOCK` | дефолт, без агрессии |
| `OURS` | лёгкий стиль, broadcast kick |
| `PAN`  | агрессивный, bidirectional + EAPOL + PMKID |
| `CUSTOM` | read-only флаг — взводится автоматически, когда ты крутишь любую ручку вручную |

Если выбрать `STOCK`/`OURS`/`PAN` — параметры перезаписываются по пресету.
...
Это значит, что твоя методика может **одинаково работать** в любом из
трёх режимов мощности: юзер берёт `PAN` для агрессии, `OURS` для тихого
режима, или руками выкручивает `KICK N=6` + `EAPOL TX=on` — методика
просто читает `ctx.bidirKick / ctx.eapolTx / ctx.kickBurst / ...` и
решает что делать. Никакой логики пресетов внутри методики быть не должно.
